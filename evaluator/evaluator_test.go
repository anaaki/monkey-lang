...
func TestFunctionApplication(t *testing.T) {
	tests := []struct {
		...
		{"let identity = fn(x) { x; }; identity(5);", 5}, // 暗黙の戻り値
		// {"let identity = fn(x) { return x; }; identity(5);", 5}, // returnの戻り値
...
func TestHashLiterals(t *testing.T) {
	...
	if len(result.Paris) != len(expected) {
		t.Fatalf("Hash has wrong num of pairs. got=%d", len(result.Paris))
	}

	for expectedKey, exexpectedValue := range expected {
		pair, ok := result.Paris[expectedKey]
		if !ok {
